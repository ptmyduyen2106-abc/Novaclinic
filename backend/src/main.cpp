...
loadEnv(const std::string& path = ".env") {
    ...
    while (std::getline(file, line)) {
        // Strip \r (Windows CRLF line endings)
        ...
        // Strip \r again after potential comment strip
...
int main(int argc, char* argv[]) {
    ...
    // Debug: kiểm tra secret load đúng không
    std::cerr << "[main] JWT Secret length: " << jwtSecret.size() << "\n";
    if (jwtSecret.size() >= 4)
        std::cerr << "[main] JWT Secret tail: ..." << jwtSecret.substr(jwtSecret.size() - 4) << "\n";

    bool configOk = true;
    if (supabaseUrl.empty())    { std::cerr << "[main] ERROR: SUPABASE_URL is required\n";            configOk = false; }
    if (serviceRoleKey.empty()) { std::cerr << "[main] ERROR: SUPABASE_SERVICE_ROLE_KEY is required\n"; configOk = false; }
    if (jwtSecret.empty())      { std::cerr << "[main] ERROR: SUPABASE_JWT_SECRET is required\n";     configOk = false; }
