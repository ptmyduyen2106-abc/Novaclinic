...
// ─────────────────────────────────────────────
// Specialization cho T = void
// Không có field "data" — dùng named constructors
// thay vì aggregate init để tránh lỗi compiler.
// ─────────────────────────────────────────────
template <>
struct DbResult<void> {
    ...
    static DbResult<void> err(std::string e, int code = 500) {
        DbResult<void> r;
        r.success    = false;
        r.error      = std::move(e);
        r.httpStatus = code;
